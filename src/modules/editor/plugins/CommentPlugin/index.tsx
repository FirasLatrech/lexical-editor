  ...
  return (
    <LexicalComposer initialConfig={initialConfig}>
      <div className="CommentPlugin_CommentInputBox_EditorContainer">
        <PlainTextPlugin
          contentEditable={
            <LexicalContentEditable placeholder={placeholder} className={className || ''} />
          }
      ...
      if ($isRangeSelection(selection)) {
        ...
        if (range !== null && boxElem !== null) {
          ...
          for (let i = 0; i < selectionRectsLength; i++) {
            const selectionRect = selectionRects[i] as DOMRect
            let elem: HTMLSpanElement = elements[i] as HTMLSpanElement
            if (elem === undefined) {
          ...
          for (let i = elementsLength - 1; i >= selectionRectsLength; i--) {
            const elem = elements[i] as HTMLSpanElement
            ...
            onClick={() => {
              showModal('Delete Comment', (onClose) => (
                <ShowDeleteCommentOrThreadDialog
                  commentOrThread={comment as Comment}
...
export default function CommentPlugin({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < activeIDs.length; i++) {
      const id = activeIDs[i] as string
    ...
    return () => {
      for (let i = 0; i < changedElems.length; i++) {
        const changedElem = changedElems[i] as HTMLElement
